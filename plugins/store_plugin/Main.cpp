...
EXPORT void UserCmd_Help(uint iClientID, const std::wstring &wscParam) {
    PrintUserCmdText(iClientID, L"/sinfo <--- will show if you have enything "
                                L"stored in the current system");
    PrintUserCmdText(
        iClientID,
        L"/store <n> <ammount> <--- <n> = the number /enumcargo supplys");
    PrintUserCmdText(
        iClientID,
        L"/unstore <n> <ammount> <--- <n> = the number /sinfo supplys");
    PrintUserCmdText(
        iClientID, L"/enumcargo <--- shows slot number of cargo in your hold");
}
...
void UserCmd_Store(uint iClientID, const std::wstring &wscParam) {
    ...
    // Check target is a Storage Container
    uint iType;
        ...
        for (auto &cargo : lstCargo) {
            ...
            if (!cargo.bMounted && gi->iIDS) {
                ...
                IniWrite(scUserStore, wstos(HkGetPlayerSystem(iClientID)),
                         ...
                         std::to_string(iGoods + cargo.iCount));
    ...
    if (wscCount <= 0) {
        PrintUserCmdText(
            iClientID, L"Error: You cannot transfer negative or zero amounts");
        return;
    }
    if (wscGoods == 0 || wscCount == 0) {
        PrintUserCmdText(
            iClientID,
            L"You must enter the right values try /enumcargo id amount");
        return;
    }
    std::list<CARGO_INFO> lstCargo;
    int iRem;
    ...
    for (auto &cargo : lstCargo) {
        if (cargo.iID == wscGoods) {
            ...
            int iGoods = 0;
            iGoods = IniGetI(scUserStore, wstos(HkGetPlayerSystem(iClientID)),
                             std::to_string(cargo.iArchID), 0);
            IniWrite(scUserStore, wstos(HkGetPlayerSystem(iClientID)),
                     std::to_string(cargo.iArchID),
...
void UserCmd_Ustore(uint iClientID, const std::wstring &wscParam) {
    ...
    if (wscGoods == 0 || iCount == 0) {
        PrintUserCmdText(
            iClientID,
            L"You must enter the right values try /ustore id amount");
        return;
    }
    uint iShip;
    pub::Player::GetShip(iClientID, iShip);
    ...
    int iGoods = IniGetI(scUserStore, wstos(HkGetPlayerSystem(iClientID)),
...
void UserCmd_Istore(uint iClientID, const std::wstring &wscParam) {
    ...
    for (auto &it3 : lstGoods) {
        ...
        if (iGoods > 0) {
            ...
            Archetype::Gun *gun = (Archetype::Gun *)eq;
            if (gun->iArchID) {
                PrintUserCmdText(iClientID, L"%u=%s=%s", count,
                                 HkGetWStringFromIDS(gun->iIdsName).c_str(),
                                 stows(it3.scValue).c_str());
            } else {
                PrintUserCmdText(iClientID, L"%u=%s=%s", count,
...
void UserCmd_EnumCargo(uint iClientID, const std::wstring &wscParam) {
    ...
    for (auto &it : lstCargo) {
        ...
        if (!it.bMounted) {
            if (gun->iArchID) {
                PrintUserCmdText(iClientID, L"%u: %s=%u", it.iID,
                                 HkGetWStringFromIDS(gun->iIdsName).c_str(),
                                 it.iCount);
            } else {
                PrintUserCmdText(iClientID, L"%u: %s=%u", it.iID,
    ...
    if (!iNum)
        PrintUserCmdText(iClientID,
                         L"Error: you have no unmounted equipment or goods");
}
