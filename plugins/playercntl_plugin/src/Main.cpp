...
void __stdcall BaseEnter_AFTER(unsigned int iBaseID, unsigned int iClientID) {
  float fValue;
  if (HKGetShipValue((const wchar_t *)Players.GetActiveCharacterName(iClientID),
                     fValue) == HKE_OK) {
...
void __stdcall RequestBestPath(unsigned int p1, DWORD *p2, int p3) {
  ...
  try {
    ...
    AddLog("ERROR: Exception in RequestBestPath p1=%d p2=%08x %08x %08x %08x "
...
EXPORT PLUGIN_INFO *Get_PluginInfo() {
  ...
  p_PI->lstHooks.push_back(
      PLUGIN_HOOKINFO((FARPROC *)&ClearClientInfo, PLUGIN_ClearClientInfo, 0));
  p_PI->lstHooks.push_back(
  ...
  p_PI->lstHooks.push_back(PLUGIN_HOOKINFO((FARPROC *)&HkIServerImpl::BaseEnter,
  ...
  p_PI->lstHooks.push_back(PLUGIN_HOOKINFO((FARPROC *)&HkIServerImpl::GFGoodBuy,
  ...
  p_PI->lstHooks.push_back(PLUGIN_HOOKINFO((FARPROC *)&HkIServerImpl::SetTarget,
  ...
  p_PI->lstHooks.push_back(
      PLUGIN_HOOKINFO((FARPROC *)&UserCmd_Process, PLUGIN_UserCmd_Process, 0));
  p_PI->lstHooks.push_back(
