...
namespace Wardrobe
{
	...
	bool Wardrobe::UserCmd_ChangeCostume(uint iClientID, const std::wstring& wscCmd, const std::wstring& wscParam, const wchar_t* usage)
	{
	...
	void Timer()
	{
		while (pendingRestarts.size())
		{
			...
			try
			{
				...
				AddLog("NOTICE: User %s costume change to %u", wstos(restart.wscCharname).c_str(), restart.costume);
			}
			catch (char* err)
			{
				AddLog("ERROR: User %s costume change to %u (%s)", wstos(restart.wscCharname).c_str(), restart.costume, err);
			}
			catch (...)
			{
				AddLog("ERROR: User %s costume change to %u", wstos(restart.wscCharname).c_str(), restart.costume);
			}
		}
	}
